...
void Dungeon::makeRooms()
{
	...
	//아래 left에서 +1되는것 때문에 gridX gridY를 뺴줌
	for (int y = 0; y < gridY; ++y)
		for (int x = 0; x < gridX; ++x)
		{
			...
			if (x > 0) room.neighbors.emplace_back((x - 1) + y * gridX);//left
			if (x < gridX - 1) room.neighbors.emplace_back((x + 1) + y * gridX);//right
			if (y > 0) room.neighbors.emplace_back(x + (y-1) * gridX);//left
			if (y < gridY - 1) room.neighbors.emplace_back(x + (y+1) * gridX);//left
